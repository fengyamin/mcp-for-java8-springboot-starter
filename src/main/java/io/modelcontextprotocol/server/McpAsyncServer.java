 ...
 *
 * <p>
 * This server implements the MCP specification, enabling AI models to expose
 * tools,
 * resources, and prompts through a standardized interface. Key features
 * include:
 * <ul>
 ...
 *
 * <p>
 * This implementation uses Project Reactor for non-blocking operations, making
 * it
 * suitable for high-throughput scenarios and reactive applications. All
 * operations return
 * Mono or Flux types that can be composed into reactive pipelines.
 *
 * <p>
 * The server supports runtime modification of its capabilities through methods
 * like
 * {@link #addTool}, {@link #addResource}, and {@link #addPrompt}, automatically
 * notifying
...
public class McpAsyncServer {
	...
	private List<String> protocolVersions = new ArrayList<String>() {
		{
			add(McpSchema.LATEST_PROTOCOL_VERSION);
		}
	};
	...
	/**
	 * Create a new McpAsyncServer with the given transport provider and
	 * capabilities.
	 * 
	...
	McpAsyncServer(McpServerTransportProvider mcpTransportProvider, ObjectMapper objectMapper,
		...
		if (rootsChangeConsumers == null || rootsChangeConsumers.isEmpty()) {
			rootsChangeConsumers = new ArrayList<BiFunction<McpAsyncServerExchange, List<McpSchema.Root>, Mono<Void>>>();
			rootsChangeConsumers.add(new BiFunction<McpAsyncServerExchange, List<McpSchema.Root>, Mono<Void>>() {
				@Override
				public Mono<Void> apply(McpAsyncServerExchange exchange, List<McpSchema.Root> roots) {
	...
	/**
	 * Gracefully closes the server, allowing any in-progress operations to
	 * complete.
	 * 
		...
		return (exchange, params) -> exchange.listRoots()
				.flatMap(listRootsResult -> {
					...
					return Flux.fromIterable(rootsChangeConsumers)
							.flatMap(consumer -> consumer.apply(exchange, roots))
							.onErrorResume(error -> {
	...
	private List<McpSchema.ResourceTemplate> getResourceTemplates() {
		...
		List<ResourceTemplate> resourceTemplates = this.resources.keySet()
				...
				.map(uri -> {
					...
					template.setUriTemplate(resource.uri());
					template.setName(resource.name());
					template.setDescription(resource.description());
					template.setMimeType(resource.mimeType());
					template.setAnnotations(resource.annotations());
					return template;
				})
				.collect(Collectors.toList());
	...
	private McpServerSession.RequestHandler<McpSchema.ListPromptsResult> promptsListRequestHandler() {
		return (exchange, params) -> {
			List<McpSchema.Prompt> promptList = this.prompts.values()
					...
					.collect(Collectors.toList());
			McpSchema.ListPromptsResult result = new McpSchema.ListPromptsResult();
			result.setPrompts(promptList);
			return Mono.just(result);
		};
	}
	 ...
	 * the
	 * sampling and roots, the logging level should be set per client session and
	 * use the
	 ...
	 * map,
	 * determines the correct reference type (either prompt or resource), and
	 * constructs a
	 * fully-typed {@code CompleteRequest} instance.
	 * 
	 * @param object the raw request parameters, expected to be a Map containing
	 *               "ref" and
	 *               "argument" entries.
	...
	private McpSchema.CompleteRequest parseCompletionParams(Object object) {
		...
		if ("ref/prompt".equals(refType)) {
			...
			((McpSchema.PromptReference) ref).setType(refType);
			((McpSchema.PromptReference) ref).setName((String) refMap.get("name"));
		} else if ("ref/resource".equals(refType)) {
			ref = new McpSchema.ResourceReference();
			((McpSchema.ResourceReference) ref).setType(refType);
			((McpSchema.ResourceReference) ref).setUri((String) refMap.get("uri"));
		} else {
	...
	/**
	 * This method is package-private and used for test only. Should not be called
	 * by user
	 * code.
	 * 
